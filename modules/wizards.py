"""Streamlit UI wizards for chart creation, source management, page layout, LLM config, and workspace management.

Each wizard is a Streamlit dialog (``@st.dialog``) that provides a self-contained form for a
specific administrative task within the GenAI Dashboard.
"""
...
from modules.models import Chart, ChartTheme, DataSource, Page, User, Workspace
from modules.s3_storage import s3_client
from modules.settings import CHARTS_FOLDER, DATA_FOLDER


def clean_gemini_code(text):
    """Strip Markdown `` ```python ... ``` `` wrappers from LLM-generated code.

    Args:
        text (str): Raw text that may contain Markdown code fences.

    Returns:
        str: The code inside the fences with leading/trailing whitespace removed.
    """
    if "```python" in text:
...
def add_source_callback():
    """Session-state callback that appends a default source entry to ``wiz_sources``."""
...
def remove_source_callback(index):
    """Session-state callback that deletes a source entry at the given index.

    Args:
        index (int): Position in the ``wiz_sources`` list to remove.
    """
    if "wiz_sources" in st.session_state:
        if 0 <= index < len(st.session_state.wiz_sources):
            del st.session_state.wiz_sources[index]


@st.dialog("✨ Новый график")
def wizard_create_chart():
    """Create a new chart via a dual-mode wizard (manual prompt or AI auto-generation).

    The wizard collects a display title, data sources (existing DB sources or a fresh
    upload), a color theme, and a goal description.  It can either emit a Celery task
    for background AI code generation or save a manual prompt stub immediately.
    """
    ...
    display_title = st.text_input(
        "Название графика (видит пользователя)",
        placeholder="Динамика Выручки 2024",
    )
    ...
    try:
        ...
        available_sources = db_sources.query(DataSource).filter(
            DataSource.workspace_id == active_ws_id,
            DataSource.active,
        ).all()
    finally:
        db_sources.close()
        ...
        if available_sources:
            ...
            sel_ids = st.multiselect(
                ...
                format_func=lambda x: (
                    f"{'📄' if src_dict[x].connector_id == 'google_sheets' else '📁'} "
                    f"{src_dict[x].filename}"
                ),
            )
    ...
    with tab_up:
        up_file = st.file_uploader(
            "Загрузить файл", type=["csv", "xlsx"], label_visibility="collapsed",
        )
    ...
    def theme_manager_fragment():
        """Inline fragment for selecting, editing, and managing chart color themes."""
        db = SessionLocal()
        try:
            ...
            with col_sel:
                sel_name = st.selectbox(
                    "🎨 Цветовая палитра:", theme_names, index=default_idx, key="theme_selector",
                )
            ...
            with col_star:
                if st.button(
                    star_icon, help=star_help,
                    key=f"star_{current_theme_obj.id}", use_container_width=True,
                ):
                    if not is_default:
            ...
            raw_colors = (current_theme_obj.colors.replace("[", "").replace("]", "")
            ...
            for i, color in enumerate(c_colors):
                with cols[i]:
                    st.markdown(
                        f'<div style="background-color:{color};width:100%;height:35px;'
                        f'border-radius:6px;border:1px solid rgba(128,128,128,0.3);"></div>',
            ...
            with st.expander("⚙️ Настроить свои палитры"):
                edit_name = st.text_input(
                    "Название темы:", value=sel_name, key=f"ed_nm_{current_theme_obj.id}",
                )
                is_dark = st.checkbox(
                    "Адаптировать для Dark Mode", value=c_dark, key=f"drk_{current_theme_obj.id}",
                )
                ...
                if bc1.button(
                    "💾 Сохранить", use_container_width=True, key=f"upd_btn_{current_theme_obj.id}",
                ):
                ...
                if bc2.button(
                    "➕ Как новую", use_container_width=True, key=f"new_btn_{current_theme_obj.id}",
                ):
                    new_t = ChartTheme(
                        user_id=user.id,
                        name=f"{edit_name} (Copy)",
                        colors=f"{nc1},{nc2},{nc3}",
                        dark_mode=is_dark,
                    )
                    db.add(new_t)
                    db.commit()
                ...
                if bc3.button(
                    "🗑️", use_container_width=True, key=f"del_btn_{current_theme_obj.id}",
                ):
                    if len(user_themes) > 1:
                        db.delete(current_theme_obj)
                        db.commit()
                        st.session_state.last_theme = (
                            theme_names[0] if theme_names[0] != sel_name else theme_names[1]
                        )
                        st.rerun(scope="fragment")
        finally:
            db.close()
    ...
    chart_controls = st.text_area(
        "Элементы управления (опционально)", placeholder="Добавить селектор выбора года.",
    )

    st.write("### 3. Выбор Интеллекта")
    from modules.llm_manager import get_providers
    ...
    if btn_auto or btn_manual:
        ...
        if up_file:
            ...
            if not os.path.exists(path):
                try:
                    s3_client.download_file("data-sources", target_filename, path)
                except Exception:
                    pass
        ...
        try:
            df_preview = (
                pd.read_csv(path, nrows=5) if path.endswith('.csv')
                else pd.read_excel(path, nrows=5)
            )
            cols_info = "\n".join(
                [f"- `{c}` ({t})" for c, t in zip(df_preview.columns, df_preview.dtypes)],
            )
        ...
        if chart_controls:
            controls_instruction = (
                f"ЭЛЕМЕНТЫ УПРАВЛЕНИЯ: {chart_controls}. "
                f"Используй st.selectbox/slider внутри render."
            )
        else:
            controls_instruction = ""
            ...
            try:
                ...
                if up_file:
                    exist_ds = db.query(DataSource).filter(
                        DataSource.filename == up_file.name,
                        DataSource.workspace_id == active_ws_id,
                    ).first()
                    if not exist_ds:
                        new_ds = DataSource(
                ...
                page = db.query(Page).filter(
                    Page.workspace_id == active_ws_id,
                    Page.name == current_page_name,
                ).first()
                if not page:
...
def wizard_manage_sources():
    """Manage data sources (local files and API connectors) for the active workspace.

    Supports adding/removing sources, toggling active state, editing connector
    configuration, replacing uploaded files, and assigning ETL handler scripts.
    """
    ...
    try:
        ...
        for src in sources_db:
            ...
            if conn_id == "base":
                ...
                conn_name = (
                    connector_class.get_meta().get("name", conn_id)
                    if connector_class else "Unknown"
                )

            if src.handler_id and src.handler_id in handlers_id_to_name:
                handler_badge = f" ➔ 🛠️ {handlers_id_to_name[src.handler_id]}"
            else:
                handler_badge = ""
            with st.expander(
                f"{'✅' if src.active else '⚪'} {src.filename} ({conn_name}){handler_badge}",
            ):
                ...
                if conn_id == "base":
                    ...
                    src.filename = st.text_input(
                        "Имя файла в системе", value=src.filename, key=f"fn_{src.id}",
                    )

                    st.write("**Заменить данные:**")
                    new_file = st.file_uploader(
                        "Загрузить новую версию (заменит текущую)",
                        type=["csv", "xlsx"], key=f"up_{src.id}", label_visibility="collapsed",
                    )
                    if new_file:
                        if st.button(
                            "🔄 Обновить файл", use_container_width=True, key=f"btn_up_{src.id}",
                        ):
                    ...
                    src.filename = col_file.text_input(
                        "Имя файла в системе", value=src.filename, key=f"fn_{src.id}",
                    )
                    ...
                    if connector_class:
                        ...
                        for f in fields:
                            ...
                            if f.get('type') == 'password':
                                temp_config[k] = st.text_input(
                                    lbl, value=str(val), type="password",
                                    key=f"cfg_{src.id}_{k}",
                                )
                            else:
                                temp_config[k] = st.text_input(
                                    lbl, value=str(val), key=f"cfg_{src.id}_{k}",
                                )
                ...
                if src.handler_id:
                    ...
                    if h_obj:
                        current_h_name = h_obj.name

                try:
                    h_idx = handlers_list.index(current_h_name)
                except Exception:
                    h_idx = 0

                sel_h_name = st.selectbox(
                    "Скрипт обработки (ETL)", handlers_list, index=h_idx,
                    key=f"h_{src.id}", help="Скрипт будет применяться к этим данным.",
                )
...
def wizard_manage_pages():
    """Manage dashboard pages -- create, rename, assign charts, and delete pages.

    Pages belong to the active workspace.  The default "Главная страница" cannot be deleted.
    """
    db = SessionLocal()
        ...
        for pg in user_pages:
            with st.expander(f"📄 {pg.name}", expanded=True):
                ...
                if set(selected_names) != set(current_selected):
                    pg.charts = [
                        c for c in available_charts if c.technical_name in selected_names
                    ]
                    db.commit()
                    st.toast(f"Обновлено для '{pg.name}'")

                if pg.name != "Главная страница":
                    with st.popover("🗑️ Удалить страницу", use_container_width=True):
                        st.warning("Удалить страницу? Графики останутся в базе.")
                        if st.button(
                            "Да, удалить", key=f"del_pg_{pg.id}",
...
def wizard_manage_llm():
    """Configure LLM provider integrations (OpenAI, DeepSeek, Gemini, etc.).

    Providers supply API keys, base URLs, and model lists used by the chart generation wizard.
    """
        ...
        if not providers:
            ...
            for name, data in providers.items():
                with st.expander(f"🔌 {name} ({data['type']})"):
                    ...
                    st.write(
                        f"**Base URL:** {data['base_url'] if data['base_url'] else 'Default'}",
                    )
    ...
    with tab_new:
        ...
        new_name = st.text_input(
            "Название интеграции", placeholder="Например: Corporate DeepSeek",
        )

        col_type, col_url = st.columns(2)
        p_type = col_type.selectbox(
            "Тип API", ["openai", "deepseek", "gemini", "other"],
            help="DeepSeek и 'other' используют формат OpenAI",
        )
        base_url = col_url.text_input(
            "Base URL (Прокси)",
            placeholder="[https://api.openai.com/v1](https://api.openai.com/v1)",
            help="Оставьте пустым для стандарта",
        )
        ...
        models_str = st.text_area(
            "Список моделей",
            placeholder="gpt-4o, gpt-3.5-turbo, deepseek-coder",
            height=100,
        )
...
def wizard_manage_workspaces():
    """Manage workspaces -- create, delete, add/remove members, and leave workspaces.

    The owner can manage members; personal workspaces (name starts with "Личное (") are
    protected from deletion.  Any authenticated user can leave a shared workspace.
    """
        ...
        for ws in current_user.workspaces:
            ...
            with st.expander(f"🏢 {ws.name} ({role_text})"):
                ...
                if is_owner:
                    ...
                    st.write(
                        "**Участники:**",
                        ", ".join([user_dict[uid] for uid in current_members]),
                    )
